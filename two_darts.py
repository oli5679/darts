...
best_strategies_list.append({'strategy':'double one','expectation':single_one_expectation})
def evaluate_all_singles(current_score):
	possible_single_strategies = []
	for number in range(1,21):
		if(current_score - number >  1):
			strategy_expectation = delta * best_strategies_list[current_score-number]['expectation']
...
def evaluate_all_doubles(current_score):
	...
	for number in range(1,21):
		if(current_score - 2*number > -1 and current_score - 2* number != 1):
			a = (pdd * best_strategies_list[current_score - 2*number]['expectation'])+ (pds * best_strategies_list[current_score- number]['expectation'])
...
def evaluate_all_triples(current_score):
	...
	for number in range(1,21):
		if(current_score - 3*number > 1):
			strategy_expectation = delta * ((ptt * best_strategies_list[current_score-3*number]['expectation'])+(1-ptt)*best_strategies_list[current_score-number]['expectation'])
...
def evaluate_bull(current_score):
	if(current_score < 50 or current_score == 51):
		...
		bullseye_expectation = delta * (pbb * best_strategies_list[current_score-50]['expectation'] + (1-pbb) * best_strategies_list[current_score-25]['expectation'])
